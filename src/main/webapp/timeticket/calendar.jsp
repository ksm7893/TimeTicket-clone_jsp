			...
			<div class="popup_warp">
				...
				<form action="" id="regiform" name="regiform" method="post">
					<div class="time_select selectBox" style="display: none">
						<p class="selectbox_title" style="display: block;">시간선택
...
function buildCalendar() {
    ...
    for (let nowDay = firstDate; nowDay <= lastDate; nowDay.setDate(nowDay.getDate() + 1)) {   // day는 날짜를 저장하는 변수, 이번달 마지막날까지 증가시키며 반복  
        ...
        if (nowDay < today) {                       // 지난날인 경우
        	...
        	/* nowColumn.data("handler","selectDay");
        	nowColumn.data("event","click");
        	nowColumn.data("month",nowDay.getMonth());
        	nowColumn.data("year",nowDay.getFullYear()); */
...
function choiceDate(nowColumn) {
    ...
    //var params="year="+$(this).parent().attr("data-year")+"&month="+$(this).parent().attr("data-month")+"&date="+$(this).attr("data-date")+"&tic_code="+${param.tic_code};
     var params="year=2023&month=06&date=17&tic_code=tic_1";
	$.ajax({
        ...
        success:function (data, textStatus, jqXHR){
        	...
        	$(data.otime).each(function(i, elem) {
        		let otime = `<button type='button' id='time_btn' class='time_btn' value='\${elem}'>
